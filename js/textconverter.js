	...
	initHelp () {
		...
		$helpButton.click(() => {
			...
			const $getSampleBtn = (btnText, sample) => {
				return $(`<button class="btn btn-default btn-sm mr-2 mb-2">${btnText}</button>`).on("click", () => this.inText = sample.trim());
			}
			...
			$getSampleBtn("Sample Hazard", ConverterUi.SAMPLE_HAZARD).appendTo($wrpSamples);
