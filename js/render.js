...
function Renderer() {
	...
	this._renderTable = function (entry, textStack, meta, options) {
		...
		if (entry.intro) {
			...
			for (let i = 0; i < len; ++i) {
				...
				this._recursiveRender(entry.intro[i], textStack, meta, {
					prefix: `<div class="pf2-table-intro ${styles}">`,
					suffix: "</div>"
				});
			}
		}
		...
		if (entry.footnotes != null) {
			...
			for (let i = 0; i < len; ++i) {
				...
				this._recursiveRender(entry.footnotes[i], textStack, meta, {
					prefix: `<div class="pf2-table-footnote ${styles}">`,
					suffix: "</div>"
				});
			}
		}
		if (entry.outro) {
			...
			for (let i = 0; i < len; ++i) {
				...
				this._recursiveRender(entry.outro[i], textStack, meta, {
					prefix: `<div class="pf2-table-outro ${styles}">`,
					suffix: "</div>"
				});
			}
		}
		...
		if (entry.entries) {
			...
			for (let i = 0; i < len; ++i) {
				...
				this._recursiveRender(entry.entries[i], textStack, meta, {
					prefix: `<p class="pf2-sidebar-text">`,
					suffix: `</p>`
				});
				meta.depth = cacheDepth;
			}
	...
	this._renderPf2SampleBox = function (entry, textStack, meta, options) {
		...
		if (entry.entries) {
			...
			for (let i = 0; i < len; ++i) {
				...
				meta.depth = 2;
				this._recursiveRender(entry.entries[i], textStack, meta, {
					prefix: '<p class="pf2-sample-box-text">',
					suffix: "</p>"
				});
				meta.depth = cacheDepth;
			}
	...
	this._renderPf2TipsBox = function (entry, textStack, meta, options) {
		...
		if (entry.entries) {
			...
			for (let i = 0; i < len; ++i) {
				...
				meta.depth = 2;
				this._recursiveRender(entry.entries[i], textStack, meta, {
					prefix: '<p class="pf2-tips-box-text">',
					suffix: "</p>"
				});
				meta.depth = cacheDepth;
			}
	...
	this._renderPf2RedBox = function (entry, textStack, meta, options) {
		...
		if (entry.entries) {
			...
			for (let i = 0; i < len; ++i) {
				...
				meta.depth = 2;
				this._recursiveRender(entry.entries[i], textStack, meta, {
					prefix: "<p class='pf2-red-box-note'>",
					suffix: "</p>"
				});
				meta.depth = cacheDepth;
			}
		...
		if (entry.ability != null) {
			...
			for (let i = 0; i < len; ++i) {
				...
				this._recursiveRender(entry.ability[i], textStack, meta, {
					prefix: "<p class='pf2-key-abilities-text'>",
					suffix: "</p>"
				});
				meta.depth = cacheDepth;
			}
	...
	this._renderSpellcasting_getEntries = function (entry) {
		...
		if (entry.constant || entry.will || entry.rest || entry.daily || entry.weekly || entry.ritual) {
			...
			if (entry.ritual && !hidden.has("ritual")) tempList.items.push({
				type: "itemSpell",
				name: `Rituals:`,
...
Renderer.creature = {
	...
	getAbilityMods(cr) {
		...
		renderStack.push(Renderer.get().render(`{@d20 ${cr.ability_modifiers.Dex}||Dexterity}`))
		renderStack.push(`<span>, <strong>Con </strong></span>`)
		renderStack.push(Renderer.get().render(`{@d20 ${cr.ability_modifiers.Con}||Constitution}`))
		renderStack.push(`<span>, <strong>Int </strong></span>`)
		renderStack.push(Renderer.get().render(`{@d20 ${cr.ability_modifiers.Int}||Intelligence}`))
		renderStack.push(`<span>, <strong>Wis </strong></span>`)
		renderStack.push(Renderer.get().render(`{@d20 ${cr.ability_modifiers.Wis}||Wisdom}`))
		renderStack.push(`<span>, <strong>Cha </strong></span>`)
		renderStack.push(Renderer.get().render(`{@d20 ${cr.ability_modifiers.Cha}||Charisma}`))
		renderStack.push(`</div>`)
		return renderStack.join("")
	},
...
Renderer.hover = {
	...
	async pCacheAndGet(page, source, hash, opts) {
		...
		switch (page) {
			...
			case UrlUtil.PG_FEATS:
				return Renderer.hover._pCacheAndGet_pLoadSimple(page, source, hash, opts, "feats.json", "feat");
	...
	async _pCacheAndGet_pLoadSingleBrew(page, opts, listProps, fnMutateItem) {
		...
		listProps.forEach(lp => {
			if (brewData[lp]) Renderer.hover._pCacheAndGet_populate(page, brewData, lp, {
				fnMutateItem,
	...
	_pCacheAndGet_handleSingleData(page, opts, data, listProps, fnMutateItem) {
		if (listProps instanceof Array) listProps.forEach(prop => data[prop] && Renderer.hover._pCacheAndGet_populate(page, data, prop, {
			fnMutateItem,
			fnGetHash: opts.fnGetHash
		}));
	...
	async _pCacheAndGet_pDoDereferenceNestedAndCache(entities, page, fnGetHash) {
		...
		if (entriesWithRefsVals.length) JqueryUtil.doToast({
			type: "danger",
	...
	$getHoverContent_generic(toRender, opts) {
		...
		return $$`<div class="stats ${opts.isBookContent || opts.isLargeBookContent ? "stats--book" : ""} ${opts.isLargeBookContent ? "stats--book-large" : ""}">${Renderer.hover.getGenericCompactRenderedString(toRender, opts.depth || 0)}</div>`;
	},
