...
function Renderer () {
	...
	this._renderAttack = function (entry, textStack, meta, options) {
		...
		textStack[0] += `<p class="pf2-stat pf2-stat__section attack">
			...
			${entry.traits != null ? ` ${renderer.render(`(${entry.traits.map((t) => `{@trait ${t}}`).join(", ")})`)}` : ""}, <strong>Damage&nbsp;</strong>${renderer.render(entry.damage)}${entry.noMAP ? "; no multiple attack penalty" : ""}</p>`;
	};
	...
	getSpellcasting (cr) {
		if (cr.spellcasting != null) {
			...
			for (let sc of cr.spellcasting) {
				...
				if (sc.attack != null) {
					renderStack.push(renderer.render(`<span>, attack </span>{@hit ${sc.attack}||Spell attack}`))
				}
...
Renderer.vehicle = {
	getCompactRenderedString (it, opts) {
		...
		<p class="pf2-stat pf2-stat__section"><strong>Piloting Check&nbsp;</strong>${it.pilotingCheck.length > 1 ? `${it.pilotingCheck.slice(0, -1).map(c => `${c.entry ? `${renderer.render(c.entry)}` : `${c.skill.includes("Lore") ? renderer.render(`{@skill Lore|CRB|${c.skill}}`) : renderer.render(`{@skill ${c.skill}}`)} (DC ${c.dc})`}`).join(", ")} or ${it.pilotingCheck.map(c => `${c.entry ? `${renderer.render(c.entry)}` : `${c.skill.includes("Lore") ? renderer.render(`{@skill Lore|CRB|${c.skill}}`) : renderer.render(`{@skill ${c.skill}}`)} (DC ${c.dc})`}`).slice(-1)}` : it.pilotingCheck.map(c => `${c.entry ? `${renderer.render(c.entry)}` : `${c.skill.includes("Lore") ? renderer.render(`{@skill Lore|CRB|${c.skill}}`) : renderer.render(`{@skill ${c.skill}}`)} (DC ${c.dc})`}`)}</p>
		${Renderer.utils.getDividerDiv()}
		${defensesStack.join("")}
		${Renderer.utils.getDividerDiv()}
		<p class="pf2-stat pf2-stat__section"><strong>Speed&nbsp;</strong>${it.speed.type === "special" ? it.speed.entry : `${it.speed.type} ${it.speed.speed} feet ${it.speed.traits ? `(${renderer.render(it.speed.traits.map(t => `{@trait ${t}}`).join(", "))})` : ""} ${it.speed.note ? `(${renderer.render(it.speed.note)})` : ""}`}</p>
