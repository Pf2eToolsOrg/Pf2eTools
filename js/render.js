...
Renderer.vehicle = {
	getCompactRenderedString (it, opts) {
		...
		<p class="pf2-stat pf2-stat__section"><strong>Piloting Check&nbsp;</strong>${it.pilotingCheck.length > 1 ? `${it.pilotingCheck.slice(0, -1).map(c => `${c.entry ? `${renderer.render(c.entry)}` : `${renderer.render(`{@skill ${c.skill}}`)} (DC ${c.dc})`}`).join(", ")} or ${it.pilotingCheck.map(c => `${c.entry ? `${renderer.render(c.entry)}` : `${renderer.render(`{@skill ${c.skill}}`)} (DC ${c.dc})`}`).slice(-1)}` : it.pilotingCheck.map(c => `${renderer.render(`{@skill ${c.skill}}`)} (DC ${c.dc})`)}</p>
		${Renderer.utils.getDividerDiv()}
		${defensesStack.join("")}
		${Renderer.utils.getDividerDiv()}
