...
class FilterBox extends ProxyBase {
	...
	on (identifier, fn) {
		(this._eventListeners[identifier] = this._eventListeners[identifier] || []).push(fn);
		return this;
	}

	off (identifier) {
		delete this._eventListeners[identifier];
		return this;
	}
	// endregion
	...
	render () {
		...
		$$(this._modalMeta.$modal)`<div class="split mb-2 mt-2 flex-v-center mobile__flex-col mh-100">
	...
	fireChangeEvent () {
		this._doSaveStateDebounced();
		(this._eventListeners[FilterBox.EVNT_VALCHANGE] || []).forEach(fn => fn());
	}
