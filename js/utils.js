...
ListUtil = {
	...
	handleGenericContextMenuClick: (evt, ele, $invokedOn, $selectedMenu, _, selection) => {
		switch (Number($selectedMenu.data("ctx-id"))) {
			case 0:
				ListUtil._handleGenericContextMenuClick_pDoMassPopout(evt, ele, $invokedOn, selection);
				break;
			case 1:
	...
	getCompleteFilterSources(it) {
		return it.otherSources ? [it.source].concat(it.otherSources.map(src => new FilterItem({
			item: src.source,
			isIgnoreRed: true
		}))) : it.source;
	},
				...
				await Promise.all(Object.entries(data._meta.dependencies).map(async ([prop, sources]) => {
					...
					await Promise.all(data[prop].map(async entry => {
						if (entry._copy) {
							switch (prop) {
								case "monster":
									return DataUtil.monster.pMergeCopy(flatDependencyData, entry, options);
								case "spell":
									return DataUtil.spell.pMergeCopy(flatDependencyData, entry, options);
								default:
				...
				Promise.all(data._meta.internalCopies.map(async prop => {
					...
					await Promise.all(data[prop].map(async entry => {
						if (entry._copy) {
							switch (prop) {
								case "monster":
									return DataUtil.monster.pMergeCopy(data[prop], entry, options);
								case "spell":
									return DataUtil.spell.pMergeCopy(data[prop], entry, options);
								case "item":
									return DataUtil.item.pMergeCopy(data[prop], entry, options);
								case "background":
									return DataUtil.background.pMergeCopy(data[prop], entry, options);
								case "race":
									return DataUtil.race.pMergeCopy(data[prop], entry, options);
								case "deity":
									return DataUtil.deity.pMergeCopy(data[prop], entry, options);
								default:
	...
	async _pRenderBrewScreen($appendTo, isModal, cbGetBrewOnClose) {
		...
		const $btnGet = $(`<button class="btn btn-info btn-sm">Get Homebrew</button>`)
			.click(async () => {
				...
				function getBrewDirs() {
					switch (page) {
						...
						case UrlUtil.PG_MAKE_BREW:
							return ["spell", "creature", "makebrew"];
						...
						case UrlUtil.PG_LANGUAGES:
							return ["language"];
						default:
							throw new Error(`No homebrew directories defined for category ${page}`);
					}
				}
				...
				if (collectionFiles.length) toLoads.push({
				...
				const getPageCats = () => {
					switch (page) {
						...
						case UrlUtil.PG_DEITIES:
							return ["deity"];
						case UrlUtil.PG_ITEMS:
							return ["item", "baseitem", "variant", "itemProperty", "itemType"];
						case UrlUtil.PG_REWARDS:
							return ["reward"];
						case UrlUtil.PG_PSIONICS:
							return ["psionic"];
